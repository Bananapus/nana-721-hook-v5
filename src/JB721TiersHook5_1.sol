...
contract JB721TiersHook5_1 is JBOwnable, ERC2771Context, JB721Hook, IJB721TiersHook5_1 {
    //*********************************************************************//
    // --------------------------- custom errors ------------------------- //
    //*********************************************************************//
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165, JB721Hook) returns (bool) {
        return interfaceId == type(IJB721TiersHook5_1).interfaceId || JB721Hook.supportsInterface(interfaceId);
    }
    ...
    function _currentRulesetOf(uint256 projectId) internal view returns (JBRuleset memory ruleset) {
        // slither-disable-next-line calls-loop
        (ruleset, ) = IJBController(address(DIRECTORY.controllerOf(projectId))).currentRulesetOf(projectId);
    }
