...
library Hook721DeploymentLib {
        ...
        returns (Hook721Deployment memory deployment)
    {
        deployment.hook_deployer = IJB721TiersHookDeployer(
            _getDeploymentAddress(path, "nana-721-hook-testnet", network_name, "JB721TiersHookDeployer")
        );

        deployment.project_deployer = IJB721TiersHookProjectDeployer(
            _getDeploymentAddress(path, "nana-721-hook-testnet", network_name, "JB721TiersHookProjectDeployer")
        );

        deployment.store = IJB721TiersHookStore(
            _getDeploymentAddress(path, "nana-721-hook-testnet", network_name, "JB721TiersHookStore")
        );
    }
