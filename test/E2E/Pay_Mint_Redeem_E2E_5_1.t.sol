...
import "../../src/JB721TiersHookProjectDeployer5_1.sol";
...
contract Test_TiersHook_E2E_5_1 is TestBaseWorkflow {
    ...
    JB721TiersHookProjectDeployer5_1 deployer;
    ...
    function testMintOnPayIfOneTierIsPassed(uint256 valueSent, bytes32 salt) external {
        ...
        assertEq(IJB721TiersHook5_1(dataHook).firstOwnerOf(tokenId), beneficiary);
    ...
    function testFuzzMintWithDiscountOnPayIfOneTierIsPassed(uint256 tierStartPrice, uint256 discountPercent) external {
        ...
        if (totalSupplyAfterPay < type(uint208).max) {
            if (tierStartPrice > type(uint104).max) {
                ...
                assertEq(IJB721TiersHook5_1(dataHook).payCreditsOf(beneficiary), tierStartPrice - paidForNft);
            } else {
                uint256 expectedCredits = mulDiv(tierStartPrice, discountPercent, JB721Constants.MAX_DISCOUNT_PERCENT);
                assertEq(IJB721TiersHook5_1(dataHook).payCreditsOf(beneficiary), expectedCredits);
            }
    ...
    function testNoMintOnPayWhenNotIncludingTierIds(uint256 valueSent, bytes32 salt) external {
        ...
        bytes memory metadata =
            abi.encode(bytes32(0), bytes32(0), type(IJB721TiersHook5_1).interfaceId, allowOverspending, rawMetadata);
        ...
        assertEq(IJB721TiersHook5_1(dataHook).payCreditsOf(beneficiary), valueSent);
    }
    ...
    function testMintReservedNft(uint256 valueSent, bytes32 salt) external {
        ...
        assertEq(IJB721TiersHook5_1(dataHook).STORE().numberOfPendingReservesFor(dataHook, highestTier), 0);
        ...
        IJB721TiersHook5_1(dataHook).mintPendingReservesFor(highestTier, 1);
        ...
        assertEq(IJB721TiersHook5_1(dataHook).STORE().numberOfPendingReservesFor(dataHook, highestTier), 1);

        JB721Tier memory tierBeforeMintingReserves =
            JB721TiersHook5_1(dataHook).STORE().tierOf(dataHook, highestTier, false);
    ...
    function testCashOutToken(uint256 valueSent, bytes32 salt) external {
        ...
        assertEq(IJB721TiersHook5_1(dataHook).STORE().numberOfBurnedFor(dataHook, highestTier), 1);
        ...
        assertEq(IJB721TiersHook5_1(dataHook).STORE().numberOfPendingReservesFor(dataHook, highestTier), 2);
    }
    ...
    function testCashOutAll(bytes32 salt) external {
        ...
        assertEq(
            IJB721TiersHook5_1(dataHook).STORE().numberOfPendingReservesFor(dataHook, tier),
        ...
        assertEq(IJB721TiersHook5_1(dataHook).STORE().numberOfBurnedFor(dataHook, tier), 5);
