...
contract Test_afterPayRecorded_Unit_5_1 is UnitTestSetup {
        ...
        public
    {
        ...
        ForTest_JB721TiersHook5_1 hook = _initializeForTestHook5_1(1); // Initialize with 1 default tier.
        ...
        if (reserveFrequency > 0 && initialSupply - nftsToMint > 0) {
            ...
            // Set up mocks for currentRulesetOf (needed for 5_1 version)
            _setupDefaultRulesetMocks5_1();
    ...
    function test_afterPayRecorded_revertsOnAmountBelowPriceIfNoMetadataAndOverspendingIsPrevented() public {
        JB721TiersHook5_1 hook = _initHookDefaultTiers5_1(10, true);
    ...
    function test_afterPayRecorded_doesNotRevertOnAmountBelowPriceIfNoMetadata() public {
        JB721TiersHook5_1 hook = _initHookDefaultTiers5_1(10);
        ...
        assertEq(hook.payCreditsOf(beneficiary), tiers[0].price - 1);
    }
    ...
    function test_afterPayRecorded_mintCorrectTier() public {
        ...
        // Pass the hook ID.
        bytes4[] memory ids = new bytes4[](1);
        ids[0] = metadataHelper.getId("pay", address(hookOrigin5_1));
    ...
    function test_afterPayRecorded_mintCorrectTierWithAnotherCurrency() public {
        ...
        JB721TiersHook5_1 hook = _initHookDefaultTiers5_1(10, false, 2, 9, jbPrice);
    ...
    function test_afterPayRecorded_revertIfUnexpectedLeftoverAndOverspendingPrevented(bool prevent) public {
        JB721TiersHook5_1 hook = _initHookDefaultTiers5_1(10, prevent);
    ...
    function test_transferFrom_revertTransferIfPausedInRuleset() public {
        ...
        // Set up persistent mocks for controller (needed for transfers in 5_1)
        vm.mockCall(
            mockJBDirectory,
        ...
        IERC721(hook).transferFrom(beneficiary, address(696_969_420), tokenId);
    }
    ...
    function test_transferFrom_pauseFlagOverridesRuleset() public {
        ...
        assertEq(IERC721(hook).ownerOf(tokenId), address(696_969_420));
    }
